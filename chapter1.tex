...
\index{S!Software elegance,
   history of!structured programming|see{Structured programming}}
...
\index{S!Structured programming!hierarchical input-process-output designing|(}
...
\index{F!FORTH!information-hiding|(}
...
Parnas\index{P!Parnas, David} proposes two criteria for decomposition:
...
\Chap{A} for an explanation.)  The words \forth{HURRIED?},
...
\index{F!FORTH!implicit data passing|(}
...
\index{F!FORTH!component programming|(}
...
\index{S!Structured programming!vs. component programming|(}
...
\index{F!FORTH!component programming|)}\index{C!Component programming|)}
...
\index{L!Lexicons|)}\index{F!FORTH!lexicon|)}
...
Suppose we define a variable called \forth{APPLES}, like this:
\begin{Code}
VARIABLE APPLES
\end{Code}
...
APPLES ? 20 ok
\end{Code}
%ARN - "20 ok" should be underlined
...
Here we've redefined \forth{APPLES.} Now it fetches the contents of a
variable called \forth{COLOR.} \forth{COLOR} is a pointer, either to
...
\index{F!FORTH!high-level language issue|(}
\index{H!High-level languages:!FORTH as|(}
...
%ARN - the original didn't have the inteview bar here
...
``understands'' a word. It understands the word \forth{DUP}, perhaps
more profoundly than you do, because there's never any question in its
mind what \forth{DUP} means.
...
\emph{compiling} words---like \forth{IF} and \forth{THEN} that provide
...
Mark Bernstein \index{B!Bernstein, Mark|(}
...
\index{F!FORTH!as design language|(}
...
\index{D!Data stacks}
\index{F!FORTH!data handling|see{Data stacks, Data structures}}
...
FORTH\index{C!Capability}
FORTH\index{F!FORTH!capability of}
