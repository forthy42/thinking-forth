...
use {}``GO TO'' commands\index{G!GOTO commands}. When these capabilities are used in the
...
\wtexfigt{fig1-1}{Unstructured code\index{G!GOTO commands},
using jumps or {}``GOTOs.''}
...
\cite{dahl72}. Each piece is called a module. Programs consist of modules
within modules.
...
Refinement'' \cite{wirth71}. The idea is that you begin by writing a skeletal
...
``Structured Design'' \cite{stevens74}. One of its principles is stated in
this paragraph: 
...
An acceptable form of coupling\index{D!Data coupling}
is {}``data coupling,'' which involves
...
In a paper \cite{parnas72} published back in 1972, Dr. David L. Parnas showed
...
the term {}``data abstraction'' \cite{liskov75}.\index{A!Abstraction}
Their example is a push-down
...
page \pageref{make-breakfast}. (If you're new to FORTH, refer to \Chap{A}
for an explanation.)  The words HURRIED?, CEREAL, EGGS, and CLEAN are
(most likely) also defined. as colon definitions. Up to a point, FORTH
exhibits all the traits we've studied: mnemonic value,
abstraction,\index{A!Abstraction} power, structured control operators,
strong functional binding, limited coupling, and modularity. But
regarding modularity, we encounter what may be FORTH's most
...
just slicing but \emph{dicing}\index{D!Dicing}.
...
for \emph{creating} app\-li\-ca\-tion-oriented languages. (That last sentence
...
special-purpose procedure library in the laboratory \cite{bern83}: {}``The
...
it strikes out. On the question of abstraction\index{A!Abstraction}
and power, it seems to be of \emph{infinite} level---supporting
everything from bit manipulation at an output port to business
applications.
...
known as {}``threaded code'' \cite{bell72}, \cite{dewar}, \cite{kogge82}. The penalty
...
FORTH\index{C!Capability} can do anything any other language can
do---usually easier.
...
\cite{dumse}. The chip includes not only hardware features but also the
...
\begin{references}{99}
\bibitem{dahl72} O.J. Dahl, E.W. Dijkstra, and C.A.R. Hoare,
Structured Programming, London, Academic Press, 1972.
\bibitem{wirth71} Niklaus Wirth, {}``Program Development by Stepwise
Refinement,''{} Com- munications of ACM, 14, No. 4 (1971), 221-27.
\bibitem{stevens74} W.P. Stevens, G.J. Myers, and L.L. Constantine,
{}``Structured Design,''{} IBM Systems Journal, Vol. 13, No. 2, 1974.
\bibitem{parnas72} David L. Parnas, {}``On the Criteria To Be Used in
Decomposing Systems into Modules,''{} Communications of the ACM,
December 1972.
\bibitem{liskov75} Barbara H. Liskov and Stephen N. Zilles,
{}``Specification Techniques for Data Abstractions,''{} IEEE
Transactions on Software Engineering, March 1975.
\bibitem{parnas79} David L. Parnas, {}``Designing Software for Ease of
Extension and Contraction,''{} IEEE Transactions on Software
Engineering, March 1979.
\bibitem{shorre71} Dewey Val Shorre, {}``Adding Modules to FORTH,''{}
1980 FORML Proceedings, p. 71.
\bibitem{bern83} Mark Bernstein, ``Programming in the Laboratory,''
  unpublished paper, 1983.
\bibitem{bell72} James R. Bell, ``Threaded Code,'' Communications
of ACM, Vol. 16, No. 6, 370-72.
\bibitem{dewar} Robert B.K. DeWar, ``Indirect Threaded Code,''
Communications of ACM, Vol. 18, No. 6, 331.
\bibitem{kogge82} Peter M. Kogge, ``An Architectural Trail to
Threaded-Code Systems,'' Computer, March, 1982.
\bibitem{dumse} Randy Dumse, ``The R65F11 FORTH Chip,'' FORTH
