...
treated as a single element by operators such as \forth{2DUP}, \forth{20VER},
etc.)

In your ordinary lexicon of stack operators, \forth{ROT} is the only one
that gives you access to the third stack item. Aside from \forth{PICK} and
\forth{ROLL} (which we'll comment on soon), there's no easy way to get at
anything below that.
...
...  DO  I LOAD  LOOP ;
\end{Code}
that is, we'll put \forth{LOAD} in a loop, then arrange for the index and limit
...
lo hi
\end{Code}
where ``lo'' and ``hi'' are the \emph{offsets} from \forth{BLK}.

We need to permute them for \forth{DO}, like this:
...
BLK @  DUP >R  + 1+  SWAP R> + +
\end{Code}
(See ``The Stylish Return Stack,'' coming up next.) Here we've \forth{DUP}ed
...
have one already: \forth{BLK}. So we try:

\begin{Code}
BLK @  + 1+  SWAP BLK @ +
\end{Code}
Now it's more readable, but it's still rather long, and redundant too.
\forth{BLK @ +} appears twice.

``\forth{BLK @ +}''? That sounds famililar. Finally our neurons connect.
...
\forth{BLK @ +} will be done on every pass of the loop. But we have created a
...
\forth{LINE} which draws a line between any two points, specified as coordinates
in this order:
...
\forth{DUP}s, \forth{OVER}s and \forth{ROT}s weren't really solving the problem,
...
manipulators, (e.g., \forth{ROT ROT}) can really eat up clock cycles. Direct
...
words \forth{VERTICAL} and \forth{HORIZONTAL} to draw these lines. Each requires
...
\end{Code}
where \forth{ORIGIN!} sets a two-element pointer to the ``origin,'' the place
where the box will start (the upper left-hand corner). Then ``\forth{30 30 BOX}''
...
Some folks like the words \forth{PICK} and \forth{ROLL}. They use these words to
access elements from any level on the stack. We don't recommend them.
For one thing, \forth{PICK} and \forth{ROLL} encourage the programmer to think
of the stack as an array, which it is not. If you have so many elements on
the stack that you need \forth{PICK} and \forth{ROLL}, those elements should be in
an array instead.
...
\end{tip}
In the stack commentary for \forth{CMOVE>} in \Fig{fig7-1}, the inner brace
represents the contents of the \forth{DO LOOP}. The stack depth upon exiting
the loop is the same as upon entering it: one element. Within the outer
braces, the stack result of the \forth{IF} clause is the same as that of the
\forth{ELSE} clause: one element left over. (What that leftover element
represents doesn't matter, as symbolized by the ``x'' next to \forth{THEN}.)
...
definition of \forth{CMOVE>} in the previous section.
...
\item If used inside a \forth{DO LOOP}, return stack operators must by symmetrical
   within the loop, and \forth{I} is no longer valid in code bounded by
   \forth{>R} and \forth{R>}.
\end{enumerate}
\end{tip}
For every \forth{>R} there must be a \forth{R>} in the same definition. Sometimes
...
resolving \forth{R>} has been skipped.
\index{D!Data stacks!return|)}
...
local variables must have its own copies. \Forth{}'s \forth{USER} variables
...
write a word that employs a \forth{DO LOOP} starting at the current position
(\forth{CURSOR @}) and ending at the zeroth position, searching for the line
feed character.
...
out the ``\forth{CURSOR @}'' and allowing the starting address to be passed as an
...
\person{Michael Ham} suggests the reason may be that beginning \Forth{} users
...
\forth{HEX}, we have to save its current value. When we're done dumping, we
restore it.
...
You still have to be careful. Going back to our \forth{DUMP} example, suppose
...
build the test for a key being pressed, and if so execute \forth{QUIT}. But what
happens?

The user starts in decimal, then types \forth{DUMP}. He exits \forth{DUMP} midway
through and finds himself, strangely, in hexadecimal.

In the simple case at hand, the best solution is to not use
\forth{QUIT}, but rather a controlled exit from the loop (via
\forth{LEAVE}, etc.) to the end of the definition where
\forth{BASE} is reset.
...
want a hex dump I say \forth{HEX DUMP}. If I want a decimal dump I say
\forth{DECIMAL DUMP}. I don't give \forth{DUMP} the privilege of messing around
with my environment.
...
\item at the point where the user may deliberately exit (just before \forth{QUIT})
...
\end{Code}
The word \forth{PUSH} takes a value from off of your data stack and ``pushes''
it onto this new stack. \forth{POP} is the opposite, ``popping'' a value from off
...
\forth{INVISIBLE}, then runs through the document determining page
...
with \forth{INVISIBLE}. If we're running the table of contents while
...
(Note that we must substitute \forth{0=} for \forth{NOT}. The '83
Standard has changed \forth{NOT} to mean one's complement, so that
\forth{1 NOT} yields true. By the way, I think this was a mistake.)
...
with \forth{CONSTANT}, not with \forth{CREATE}, using a dummy value of
...
constant \forth{POINTERS}.  (The word \forth{>BODY} converts the
...
In this model, the names \forth{TOP}, \forth{BOTTOM}, etc., can be
made to point into either of two tables, \forth{REAL} or
...
\forth{CMOVE} is required each time we save or restore the values. In this
...
\index{D!DOER/MAKE|(}
The syntax is called \forth{DOER/MAKE}. (If your system doesn't include
...
with the defining word \forth{DOER}, as in

\begin{Code}
DOER PLATFORM
\end{Code}
Initially, the new word \forth{PLATFORM} does nothing. Then you can write
words that change what \forth{PLATFORM} does by using the word \forth{MAKE}:
...
\end{Code}
\forth{RIGHTWING} will make \forth{PLATFORM} display ``opponent.'' You can use
\forth{PLATFORM} within another definition:
...
the end of \forth{LEFTWING} serves for both \forth{LEFTWING} and for the bit of
code after \forth{MAKE}. When \forth{MAKE} redirects execution of the \forth{DOER} word,
...
\forth{PLATFORM} and exits. Invoking \forth{LEFTWING} does not cause ``proponent''
to be printed. \Fig{fig7-7} demonstrates this point, using a
conceptualized illustration of the dictionary.

If you want to \emph{continue} execution, you can use the word \forth{;AND} in
place of semicolon. \forth{;AND} terminates the code that the \forth{DOER} word
...
Finally, you can chain the ``making'' of \forth{DOER} words in series by
not using \forth{;AND}. \Fig{fig7-9} explains this better than I could write about
it.
\index{V!Vectored Execution|)}

\section{Using DOER/MAKE}

There are many occasions when the \forth{DOER/MAKE} construct proves
beneficial. They are:
...
necessary). The words \forth{LEFTWING} and \forth{RIGHTWING} change the state of
the word \forth{PLATFORM}.
...
\end{Code}
The problem arises when you write a definition called \forth{CDUMP}, designed to
...
Here's a solution to this problem, using \forth{DOER/MAKE}. The code that
changes has been replaced with the word \forth{.UNIT}, whose behavior is vectored
by the code in \forth{DUMP} and \forth{CDUMP}. (Recognize that ``\forth{1 +LOOP}'' has
the same effect as ``\forth{LOOP}''.)
...
``prime'' mark at the end. \forth{VISIBLE} sets them to their expected functions.
\forth{INVISIBLE} makes them no-ops, eating up the arguments that would normally
be passed to them. Say \forth{INVISIBLE} and any words defined in terms
...
\end{Code}
The word \forth{ANNOUNCE} will display either message. First we say \forth{LONG}, to
initialize \forth{ANNOUNCE} to the long message. Now we can test \forth{ANNOUNCE},
and find that it prints the long message. Having done that,
however, it continues to ``make'' \forth{ANNOUNCE} display the short message.

If we test \forth{ANNOUNCE} a second time, it prints the short message. And it
will for ever more, until we say \forth{LONG} again.
...
\end{Code}
In this code we've created a list of variables, then defined an \forth{ORDER} in
which they are to be searched. The word \forth{HUNT} looks through each of them,
looking for the first one that contains a 17. \forth{HUNT} returns either the
address of the correct variable, or a zero if none have the value.

It does this by simply executing \forth{WHERE} five times. Each time, \forth{WHERE}
returns a different address, as defined in \forth{ORDER}, then finally zero.

We can even define a \forth{DOER} word that toggles its own behavior endlessly:
...
To implement a forward reference, build the header of the word with \forth{DOER},
