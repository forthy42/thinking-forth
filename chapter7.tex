...
treated as a single element by operators such as \forthb{2DUP}, \forthb{20VER},
etc.)
...
...  DO  I LOAD  LOOP ;
\end{Code}
that is, we'll put \forthb{LOAD} in a loop, then arrange for the index and limit
...
``\forthb{BLK @ +}''? That sounds famililar. Finally our neurons connect.
...
\forthb{BLK @ +} will be done on every pass of the loop. But we have created a
...
\forthb{DUP}s, \forthb{OVER}s and \forthb{ROT}s weren't really solving the problem,
...
manipulators, (e.g., \forthb{ROT ROT}) can really eat up clock cycles. Direct
...
Some folks like the words \forthb{PICK} and \forthb{ROLL}. They use these words to
access elements from any level on the stack. We don't recommend them.
For one thing, \forthb{PICK} and \forthb{ROLL} encourage the programmer to think
of the stack as an array, which it is not. If you have so many elements on
the stack that you need \forthb{PICK} and \forthb{ROLL}, those elements should be in
an array instead.
...
\end{tip}
In the stack commentary for \forthb{CMOVE>} in \Fig{fig7-1}, the inner brace
represents the contents of the \forthb{DO} \forthb{LOOP}. The stack depth upon exiting
the loop is the same as upon entering it: one element. Within the outer
braces, the stack result of the \forthb{IF} clause is the same as that of the
\forthb{ELSE} clause: one element left over. (What that leftover element
represents doesn't matter, as symbolized by the ``x'' next to \forthb{THEN}.)
...
\item If used inside a \forthb{DO} \forthb{LOOP}, return stack operators must by symmetrical
   within the loop, and \forthb{I} is no longer valid in code bounded by
   \forthb{>R} and \forthb{R>}.
\end{enumerate}
\end{tip}
For every \forthb{>R} there must be a \forthb{R>} in the same definition. Sometimes
...
local variables must have its own copies. \Forth{}'s \forthb{USER} variables
...
write a word that employs a \forthb{DO} \forthb{LOOP} starting at the current position
...
\forthb{HEX}, we have to save its current value. When we're done dumping, we
restore it.
...
You still have to be careful. Going back to our \forthb{DUMP} example, suppose
...
build the test for a key being pressed, and if so execute \forthb{QUIT}. But what
happens?

The user starts in decimal, then types \forthb{DUMP}. He exits \forthb{DUMP} midway
...
want a hex dump I say \forthb{HEX DUMP}. If I want a decimal dump I say
\forthb{DECIMAL DUMP}. I don't give \forthb{DUMP} the privilege of messing around
with my environment.
...
\item at the point where the user may deliberately exit (just before \forthb{QUIT})
...
\index{D!DOER/MAKE|(}%
The syntax is called \forth{DOER}/\forth{MAKE}. (If your system doesn't include
...
with the defining word \forthb{DOER}, as in
...
code after \forth{MAKE}. When \forth{MAKE} redirects execution of the \forthb{DOER} word,
...
If you want to \emph{continue} execution, you can use the word \forthb{;AND} in
place of semicolon. \forthb{;AND} terminates the code that the \forthb{DOER} word
...
not using \forthb{;AND}. \Fig{fig7-9} explains this better than I could write about it.%
\index{V!Vectored Execution|)}
...
\forthb{DOER}/\forthb{MAKE}. The code that changes has been replaced
with the word \forth{.UNIT}, whose behavior is vectored by the code in
\forth{DUMP} and \forth{CDUMP}. (Recognize that ``\forthb{1} \forthb{+LOOP}''
has the same effect as ``\forthb{LOOP}''.)
...
\end{Code}
Notice how \forth{DUMP} and \forth{CDUMP} \emph{set-up} the vector, then go on to
\emph{execute} the shell (the word \forth{<DUMP>}).
...
We can even define a \forthb{DOER} word that toggles its own behavior endlessly:
...
To implement a forward reference, build the header of the word with \forthb{DOER},
