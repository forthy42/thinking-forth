...
\end{tip}
Many instances of doubly-nested \forthb{IF THEN} structures can be simplified
...
The logical ``or'' situation, when implemented with \forthb{IF THEN}s, is
even clumsier:
...
word using nested \forthb{IF ELSE THEN}s, like this:
...
same code as the nested \forthb{IF THEN} statements. A case statement is a
...
question its value. If there are only a few cases, a nested \forthb{IF ELSE THEN}
...
introduces two exit points to a \forthb{DO LOOP}.\index{D!DO LOOP}
...
the doctrine of structured programming. In a \forthb{BEGIN WHILE REPEAT}
...
limit depending on the current state. Using \forthb{IF THEN} we would write:

\begin{Code}
: SPEED-LIMIT  ( -- speed-limit)
     'FREEWAY? @  IF  55  ELSE  25  THEN ;
\end{Code}
We might eliminate the \forthb{IF THEN} by using a hybrid value with \forthb{AND}:
...
\end{Code}
Have we gained anything over the \forthb{IF THEN} approach? Probably not,
...
It's similar to the effect of an \forthb{ABORT"}, which has an \forthb{IF THEN}
built in it. But that's only one \forthb{IF THEN} in the system, not at every
error.
...
If you have an \forthb{IF ELSE THEN} phrase in which no code follows
...
\forthb{DO LOOP} that we wrote for \forth{'FUNCTION} in our Tiny Editor, earlier in
...
\begin{enumerate}
\item You have the word \forth{CHOOSE} which takes an argument ``$n$'' and returns a
