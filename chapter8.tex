...
Suppose a student wants to write a new word called \forth{B."} (for
...
The word \forth{'} (tick) has a similar history. It was \forth{STATE}-dependent in
...
\begin{tip}
Words should not depend on \forth{STATE} if a programmer might ever want to
invoke them from within a higher-level definition and expect them to
behave as they do interpretively.
\end{tip}
\forth{ASCII}\index{A!ASCII} works well as a STATE-dependent word, and so does \forth{MAKE}.
(See \App{C}.)
\index{C!Control structure minimization!dictionary|)}
\index{D!Dictionary:!control structure minimization with|)}
...
\end{tip}
Many instances of doubly-nested \forth{IF THEN} structures can be simplified
...
: ?PLAY   SATURDAY? IF  WORK FINISHED? IF
     GO PARTY  THEN  THEN ;
\end{Code}
The above code uses nested \forth{IF}s to make sure that it's both
...
The logical ``or'' situation, when implemented with \forth{IF THEN}s, is
even clumsier:
...
word using nested \forth{IF ELSE THEN}s, like this:
...
same code as the nested \forth{IF THEN} statements. A case statement is a
...
question its value. If there are only a few cases, a nested\forth{IF ELSE THEN}
...
This is a live topic, because of the multiple \forth{WHILE} construct which is in
poly\Forth{} but hasn't percolated up to \Forth{} '83. It's a simple way of
defining multiple \forth{WHILE}s in the same \forth{REPEAT}.

Also Dean Sanderson [of \Forth{}, Inc.] has invented a new construct that
introduces two exit points to a \forth{DO LOOP}.\index{D!DO LOOP}
...
later I'll have an \forth{IF} to see whether I left the loop early,
...
the doctrine of structured programming. In a \forth{BEGIN WHILE REPEAT}
loop with multiple \forth{WHILE}s, all the exits bring you to a common
``continue'' point: the \forth{REPEAT}. But with Sanderson's construct, you
can exit the loop by jumping \emph{past} the end of the loop, continuing
at an \forth{ELSE}. There are two possible ``continue'' points.
...
\forth{TYPE} can be coded:

\begin{Code}
: TYPE  ( a #)  OVER + SWAP DO  I C@ EMIT  LOOP ;
\end{Code}
(Although \forth{TYPE} really ought to be written in machine code.)

This definition uses no overt conditional. \forth{LOOP} actually hides the
conditional since each loop checks the index and returns to \forth{DO} if it
...
: TYPE  ( a)  BEGIN DUP C@  ?DUP WHILE  EMIT  1+
   REPEAT  DROP ;
\end{Code}
An extra test is needed on each pass of the loop. (\forth{WHILE} is a conditional
operator.)

Optimization note: The use of \forth{?DUP} in this solution is expensive in
...
: TYPE  ( a)  BEGIN DUP C@  DUP WHILE EMIT 1+
    REPEAT  2DROP ;
\end{Code}
The '83 Standard applied this principle to \forth{INTERPRET} which now accepts
...
\Forth{} systems. \forth{AND}ing ``n'' with the flag will either produce ``n'' (all
...
\begin{tip}
Use \forth{MIN} and \forth{MAX} for clipping.
\end{tip}
...
VALUE @  1-  O MAX  VALUE !
\end{Code}
In this case the conditional is factored within the word \forth{MAX}.
...
limit depending on the current state. Using \forth{IF THEN} we would write:

\begin{Code}
: SPEED-LIMIT  ( -- speed-limit)
     'FREEWAY? @  IF  55  ELSE  25  THEN ;
\end{Code}
We might eliminate the \forth{IF THEN} by using a hybrid value with \forth{AND}:
...
\end{Code}
Have we gained anything over the \forth{IF THEN} approach? Probably not,
...
@ EXECUTE
\end{Code}
will perform the routine (or \forth{@EXECUTE} if you have it).
...
moment we invoke \forth{HERE}, it's pointing to the next free cell after the last
...
\end{Code}
We use these names to supply the addresses passed to \forth{DO}:
\begin{Code}
'NOMATCH FUNCTION DO
\end{Code}
...
It's similar to the effect of an \forth{ABORT"}, which has an \forth{IF THEN}
built in it. But that's only one \forth{IF THEN} in the system, not at every
error.
...
\index{E!EXIT|(}
The word \forth{EXIT} terminates the definition in which the word appears.
The phrase \forth{R> DROP} terminates the definition that called the
...
If you have an \forth{IF ELSE THEN} phrase in which no code follows
\forth{THEN}, like this:

\begin{Code}
... HUNGRY?  IF  EAT-IT  ELSE  FREEZE-IT  THEN ;
\end{Code}
you can eliminate \forth{ELSE} by using \forth{EXIT}:

\begin{Code}
... HUNGRY?  IF EAT-IT EXIT  THEN  FREEZE-IT ;
\end{Code}
(If the condition is true, we eat and run; \forth{EXIT} acts like a semicolon.
If the condition is false, we skip to \forth{THEN} and FREEZE-IT.)

The use of \forth{EXIT} here is more efficient, saving two bytes and extra
...
in the middle without having to match all your \forth{THEN}s at the end. In one
...
\end{Code}
SIMPLE handled the simple cases. SIMPLE ended up with \forth{R> DROP}.
These other conditions were the more complex ones.

Everyone exited at the same point without having to painfully match all
the \forth{IF}s, \forth{ELSE}s, and \forth{THEN}s. The final result, if none of the
conditions matched, was an error condition.

It was bad code, difficult to debug. But it reflected the nature of the
problem. There wasn't any better scheme to handle it. The \forth{EXIT} and
\forth{R> DROP} at least kept things manageable.
\end{tfquot}\index{M!Moore, Charles|)}
\end{interview}
Programmers sometimes also use \forth{EXIT} to get out of a complicated
\forth{BEGIN} loop in a graceful way. Or we might use a related technique in the
\forth{DO LOOP} that we wrote for 'FUNCTION in our Tiny Editor, earlier in
...
We can introduce the word LEAP (see \App{C}), which will work
like \forth{EXIT} (it will simulate a semicolon). Now we can write:
...
\end{Code}
If we find a match we LEAP, not to \forth{+LOOP}, but right out of the definition,
...
Remember though, this use of \forth{EXIT} and \forth{R> DROP} is not consistent
...
a definition which is consumed at the end. A premature \forth{EXIT} will leave
...
\end{Code}
\begin{tip}
\forth{DUP} a flag, don't recreate it.
\end{tip}
...
the \forth{DOER} word TYPE (the pointer to the current code) and compares it
...
\forth{DOER} word vectored to perform a structured exit.
...
To \forth{IF} or not to \forth{IF}
