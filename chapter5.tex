...
Avoid embedding a \forth{LOAD} or \forth{THRU} command in the middle of a
...
\begin{description}
\item[\emph{\forth{\bs}}]
\item[\emph{\forth{\bs S}}] also called \forth{EXIT}
\end{description}

\noindent \forth{\bs} is pronounced ``skip-line.'' It causes the
...
\   SWAP ROT  (NUTATE) ;
\end{Code}
\forth{\bs S} is pronounced ``skip-screen.'' It causes the \Forth{}
interpreter to stop interpreting the screen entirely, as though there
were nothing else in the screen beyond \forth{\bs S}.

In many \Forth{} systems, this function is the same as \forth{EXIT},
which is the run-time routine for semicolon.  In these systems the use
of \forth{EXIT} is acceptable.  Some \Forth{} systems, however,
require for internal reasons a different routine for the
``skip-screen'' function.

Definitions for \forth{\bs} and \forth{\bs S} can be found in \App{C}.
\index{L!Load screens!application|)}
...
single word, \forth{FH} (see \App{C} for its definition).
...
I use \forth{LOCATE} {[}also known as \forth{VIEW}; see the Handy Hint
in \emph{Starting \Forth{},} Chapter Nine{]}.  As a result, the
listing is much less carefully organized because I have \forth{LOCATE}
...
the \forth{THRU} command in your chapter-load screen to load each
...
\end{enumerate}
The use of \forth{THRU} seems to give you the greatest control.
...
case, though, normal \forth{LOAD}ing will do just as well, since the
...
\noindent \Fig{fig5-4} shows a typical department's partitioning scheme.
...
portion of a list produced by \forth{INDEX} in which the comment lines
...
\forth{VARIABLES} and \forth{CONSTANTS} should also be defined one per
...
\begin{tip}
All screens must leave \forth{BASE} set to \forth{DECIMAL}.
\end{tip}
Even if you have three screens in a row in which the code is written
in \forth{HEX} (three screens of assembler code, for instance), each
screen must set \forth{BASE} to \forth{HEX} at the top, and restore
base to \forth{DECIMAL} at the bottom.  This rule ensures that each
...
base to \forth{DECIMAL} at the end, they reset base to \emph{whatever
...
such as the value returned by \forth{BEGIN} or \forth{IF} in a
...
changes \forth{BASE}, so they don't have to worry about it.
\index{B!BASE|)}

\person{Moore}\index{M!Moore, Charles} prefers to define \forth{LOAD}
to invoke \forth{DECIMAL} after loading.  This approach simplifies the
...
1 space between semicolon and \forth{IMMEDIATE} (if invoked)
\end{quote}
...
A \emph{stack-effect comment}\index{S!Stack-effect comment}
...
A \emph{data-structure comment}\index{D!Data-structure comment}
...
An \emph{input-stream comment}\index{I!Input-stream comment}
...
A \emph{purpose comment}\index{P!Purpose comment}
...
A \emph{narrative comment}\index{N!Narrative comments}
...
: STATUS?  ( -- status) ... ;
\end{Code}
If a \forth{2VARIABLE} contains one double-length value, the comment
should be a stack picture that indicates the contents:
\begin{Code}
2VARIABLE PRICE  \ price in cents
\end{Code}
If a \forth{2VARIABLE} contains two single-length data elements, it's
...
not include the address returned by \forth{DOES>,} even though this
...
\emph{application} is well-commented.  Line-by-line commenting doesn't
...
For instance, the \Forth{} word \forth{ALLOT} simply increments the
dictionary pointer (called \forth{DP} or \forth{H} in most systems).
But the name \forth{ALLOT} is better than \forth{DP~+!} because the
user is thinking of reserving space, not incrementing a pointer.

The '83 Standard adopted the name \forth{CMOVE>} instead of the
previous name for the same function, \forth{<CMOVE}.  The operation
...
\end{tfquot}
Henry Laxen,\index{L!Laxen, Henry}
...
The word \forth{ASCII} is used primarily within colon definitions to
...
\forth{TRUE} and \forth{FALSE}.  With these additions you can write
phrases such as
...
(I once used \forth{T} and \forth{F}\forth{,} but the words are
...
An often-used word is \forth{WITHIN},\index{W!WITHIN} which determines
...
\bibitem{dijkstra82} Edsger W. Dijkstra, \emph{Selected Writings on
Computing: A Personal Perspective,} New York, Springer Verlag, Inc.,
1982.
\bibitem{laxen} Henry Laxen, ``Choosing Names,'' \emph{FORTH Dimensions,}
