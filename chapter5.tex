...
%!!<ENUMERATION>
\begin{description}
\item[\emph{\forth{\backslash}}]
\item[\emph{\forth{\backslash S}}] also called \forth{EXIT}
\end{description}
%!!</ENUMERATION>
\forth{\backslash} is pronounced ``skip-line.'' It causes the \Forth{}
interpreter to ignore everything to the right of it on the same line.
(Since \forth{\backslash} is a \Forth{} word, it must be followed by a
space.) It does not require a delimiter.

In \Fig{fig5-1}, you see \forth{\backslash} used in two ways: to begin
...
During testing, \forth{\backslash} also serves to temporarily ``paren
...
\   SWAP ROT  (NUTATE) ;
\end{Code}
\forth{\backslash S} is pronounced ``skip-screen.'' It causes the
\Forth{} interpreter to stop interpreting the screen entirely, as
though there were nothing else in the screen beyond \forth{\backslash
S}.
...
Definitions for \forth{\backslash} and \forth{\backslash S} can be found in \App{C}.
...
If your system has \forth{\backslash} (``skip-line''-see \App{C}), you can write
...
word -TEXT: \index{A!Abbreviations!in stack diagrams}
\begin{Code}
( at u a2 -- ?)
( at u a2 -- t=no-match)
( at u a2 -- f=match)
