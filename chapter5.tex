...
%!!<ENUMERATION>
\begin{description}
\item[\emph{Screens:}] the smallest unit of FORTH source
\item[\emph{Lexicons:}] one to three screens, enough to implement a component
\item[\emph{Chapters:}] a series of related lexicons, and
\item[\emph{Load screens:}] analogous to a table of contents, a screen that loads the
...
\end{table}
\footnotetext[1]{An often-seen alternative calls for 1 space between the name and comment and 3
...
{\othersidetrue\section{Comment Conventions}}
...
text  &  sequence of characters, delimited by non-blank\footnotemark[2] \\
\end{tabular}}
\end{table*}

\footnotetext[2]{Follow ``text'' with the actual delimiter required; e.g.: text" or text)}
%!!</TABLE>
...
%!!<ENUMERATION>
\begin{description}
\item[\emph{Scanning-for}] means looking ahead in the input stream, either for a word or
number as in the case of tick, or for a delimiter as in the case of .'' and ( .
\item[\emph{Expecting}] means waiting for.  EXPECT and KEY, and definitions that invoke
them, are ones that ``expect'' input.
\item[\emph{Presuming}] indicates that in normal usage something will follow.  The word:
