...
: TIP VALUE JUDGEMENT ;
\end{Code}
I'd especially like to thank Kim Harris,\index{H!Harris, Kim}
who proposed many of the conventions described in this chapter, for
his continuing efforts at unifying divergent views on good \Forth{}
style.
...
\end{description}
%!!</ENUMERATION>
\forth{\bs} is pronounced ``skip-line.'' It causes the \Forth{}
interpreter to ignore everything to the right of it on the same line.
(Since \forth{\bs} is a \Forth{} word, it must be followed by a
space.) It does not require a delimiter.

In \Fig{fig5-1}, you see \forth{\bs} used in two ways: to begin
the screen-comment line (Line 0), and to begin comments on individual
lines which have no more code to the right of the comment.

During testing, \forth{\bs} also serves to temporarily ``paren
...
\   SWAP ROT  (NUTATE) ;
\end{Code}
\forth{\bs S} is pronounced ``skip-screen.'' It causes the
\Forth{} interpreter to stop interpreting the screen entirely, as
though there were nothing else in the screen beyond \forth{\bs
S}.

In many \Forth{} systems, this function is the same as \forth{EXIT}, which
is the run-time routine for semicolon.  In these systems the use of \forth{EXIT} is
...
word, FH (see \App{C} for its definition).
...
the application-load screen.  This permits selectively \forth{FORGET}ting any
...
\begin{interview}
\person{Charles Moore}\index{M!Moore, Charles|(} (no relation to Moore Products
Co.) places less importance on the well-organized hierarchical listing
than I do.  \person{Moore}:

\begin{tfquot}
I structure \emph{applications} hierarchically, but not necessarily
\emph{listings.}  My listings are organized in a fairly sloppy way, not at
all hierarchically in the sense of primitives first.

I use \forth{LOCATE} {[}also known as \forth{VIEW}; see the Handy Hint in
\emph{Starting \Forth{},} Chapter Nine{]}.  As a result, the listing
is much less carefully organized because I have \forth{LOCATE} to find
things for me.  I never look at listings.
...
\forth{THRU} command in your chapter-load screen to load each series of
screens, or linking each series together with the arrows and \forth{LOAD}ing
...
case, though, normal \forth{LOAD}ing will do just as well, since the assembler
...
called a ``triad''; most \Forth{} systems include the word \forth{TRIAD} to
...
Similarly, the word \forth{INDEX} lists the first line of each screen, 60 per
...
line in the disk \forth{INDEX}.  It should describe the purpose of the screen (not
...
If your system has \forth{\bs} (``skip-line''-see \App{C}), you can write
...
by \forth{INDEX} in which the comment lines for the continuing screens
are indented.
%!!<Figure>
\begin{figure*}[bbbb]
...
\person{Moore}\index{M!Moore, Charles} prefers to define \forth{LOAD} to
...
most of the commonly used abbreviations.  (This table reappears in \App{E}.)
...
pointer (called \forth{DP} or \forth{H} in most systems).  But the
name \forth{ALLOT} is better than \forth{DP~+!} because the user is
...
\forth{4-0R-9}.  That's awkward, but it was much later that I thought of
\forth{ALMOST}.
...
%!!<SMALLPRINT>
\begin{interview}
The choice of \forth{I'M} is another invention of \person{Moore}, who says:
...
%!!</SMALLPRINT>
Another of \person{Moore}'s favorite words is TH, which he uses as an array indexing
...
\person{Moore}\index{M!Moore, Charles} recommends the word UNDER+.  It's useful for adding a
