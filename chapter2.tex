...
cycle, illustrated in \Fig{fig2-1}:

\wepsfiga{fig2-1}{The iterative approach to the
software development cycle, from ``The FORTH Philosophy,''
by Kim Harris, Dr. Dobb's Journal.}
...
% Indented list of words - no bullet!! WRONG
\begin{itemize}
\item Good
\item Fast
\item Cheap
...
\wepsfigp{img2-047}{Refining the conceptual model to meet requirements and constraints.}
...
\wepsfiga{fig2-2}{An iterative approach to analysis.}
...
words: \forth{C@}, \forth{DUP}, \forth{OVER}, etc., and show only the
...
%SS left same as page #
\subsection{The Decision Tree}

\noindent \Fig{fig2-4} illustrates the telephone rate rules by means of a decision tree.
...
\wepsfiga{fig2-4}{Example of a decision tree.}
...
\wepsfiga{fig2-5}{The decision table.}
...
\wepsfiga{fig2-6}{A simplified decision table.}
...
What happens if we split this table into two tables, as in \Fig{fig2-7}?
...
appropriate rates from the table. The word \forth{1MINUTE} will get the rate for the
first minute; \forth{+MINUTES} will get the rate for each additional minute.
...
\wepsfiga{fig2-8}{The decision table without operator involvement depicted.}
...
can define the word \forth{MILEAGE-RATE}, which will fetch the mileage
rate from the table:
...
records) called \forth{BOOKS}. Then you'll have to determine what other data
structures will be required to let the users search the \forth{BOOKS} efficiently.

%Page 060 in first edition

\wepsfigp{img2-060}{Given two adequate solutions, the correct one is the simpler.}
...
% Should be outside the enumerate
