...
It} \cite{polya}. Although solving a mathematical problem isn't quite the same as
...
out working forward\index{W!Working forwards}, by mentally transferring water from one container
...
In some cases this word order\index{W!Words:!ordering} sounds unnatural. For instance, suppose
...
was written with wonderful simplicity in FORTH by Michael Stolowitz \cite{stolowitz82}.
...
``\textbf{STATE}-dependent'' (see \Chap{C}). If you don't have \textbf{ASCII},
...
algorithm(s)\index{A!Algorithms} and data structure(s), and concluding with an implementation
in FORTH.
