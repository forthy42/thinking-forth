...
problems in Chapters \ref{chapter-7} and \ref{chapter-8}.)%
\index{P!Postfix notation|)}
...
An example is \forth{."} (dot-quote), which precedes the text it will later
...
define \forth{READY}, you can do it so:
\begin{Code}
...
is named for the ninth century Persian mathematician \person{al-Khowarizimi}.)
...
by limiting the number of \forthb{IF }\forthb{THEN}s in this definition to one.
