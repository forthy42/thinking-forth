...
(For novices: The apostrophe in the definition of \forth{LIGHT} is a
\Forth{} word called ``tick.'' Tick is a dictionary-search word; it takes
a name and looks it up in the dictionary, returning the address where the
definition resides. Used in this definition, it will find the address of
the word following \forth{LIGHT}---for instance, \forth{BLUE}---and pass
this address to the word \forthb{EXECUTE}, which will execute
\forth{BLUE}, pushing a one onto the stack.  Having ``sucked up'' the
operation of \forth{BLUE}, \forth{LIGHT} now ``or''s an 8 into
the 1, producing a 9.)
...
the ninth century Persian mathematician \person{al-Khowarizimi}.)
...
In listing the symbols, we've also \emph{organized} them in a way that
seems right. The symbols in the left column are all multiples of ten; the
symbols in the right column are multiples of five. Furthermore, the
symbols in each row have ten times the value of the symbols directly above
them.
