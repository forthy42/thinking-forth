...
Segment 1:	\forth{WILLY NILLY  PUDDIN' PIE AND}

Segment 2:	\forth{WILLY NILLY  8 * PUDDING PIE AND}

\bigskip

\noindent
How can you factor out the ``\forth{8 *}'' operation? By including
...
Segment 1:    \forth{1 NEW}

Segment 2:    \forth{8 NEW}
...
%% could not get the proper spacing for the code
Segment 1:	\forth{BLETCH-A BLETCH-B BLETCH-C BLETCH-D BLETCH-E BLETCH-F}

Segment 2:	\forth{BLETCH-A BLETCH-B PERVERSITY BLETCH-D BLETCH-E BLETCH-F}
...
\noindent 
Segment 1:	\forth{TRUE BLETCHES}

\noindent
Segment 2:	\forth{FALSE BLETCHES}
...
\indent Segment 1:	\forth{BLETCH-AB BLETCH-C BLETCH-DEF}

\indent Segment 2:	\forth{BLETCH-AB PERVERSITY BLETCH-DEF}
...
Be alert to repetitions on either side of an \forth{IF ELSE THEN}
...
is a control code, it emits a dot. Either way, an \forth{EMIT} is
performed. Factor \forth{EMIT} out of the conditional structure, like
this:
...
Here's a reminder about factoring code from out of a \forth{DO LOOP}:

\begin{tip}
In factoring out the contents of a \forth{DO LOOP} into a new
...
Here are two definitions whose differences lies within a \forth{IF
THEN} construct:
...
changes. Since you can't factor the \forth{IF} into one word and the
\forth{THEN} into another, the simplest thing is to factor the
condition:
...
\end{Code}
The word \forth{CONDITIONALLY} may--depending on the condition--alter
...
statements,\index{C!Case statements} which eliminate nested \forth{IF
ELSE THEN}s, and multiple exit loops ( the \forth{BEGIN WHILE WHILE
WHILE ... REPEAT} construct). We'll also discuss these topics in
\Chap{8}.
...
\forth{INIT-CH} words has been factored into a \forth{DO LOOP}:
...
can copy the string \emph{and} the count with a single \forth{CMOVE}.
...
definition needs the address of \forth{EXIT} to conclude the
...
\forth{LIT} (or literal), another 2 bytes per definition. (If 1 and 2
...
factoring. Any time you see a doubly-nested \forth{DO LOOP}, that's a
sign that something's wrong because it will be hard to debug. Almost
always take the inner \forth{DO LOOP} and make a word.
...
the word \forth{>BODY}, you would lose transportability if you ever
...
\forth{HOLD} instead of \forth{EMIT}.)
...
last-number-plus-one and first-number order required by \forth{DO}.)
...
\begin{tfquot}
That particular phrase \forth{[OVER + SWAP]} is one that's right on
...
such a phrase, you have trouble knowing exactly what \forth{RANGE}
does. You can't see the manipulation in your mind. \forth{OVER +
SWAP} has greater mnemonic value than \forth{RANGE}.
\end{tfquot}
...
\forth{\#POINTS}, used as the limit in \forth{DRAW}'s \forth{DO LOOP}.
...
\index{D!Defining words!compile-time factoring though}
...
defining the first port, then invoke \forth{DROP} when we've finished
...
simple: instead of hard-coding the \forth{ASCII *} we'd like to change
...
\begin{references}{9}
\bibitem{stevens74-6} W.P. Stevens, G.J. Myers, and L.L. Constantine,
