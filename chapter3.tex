...
%%
\chapternum{THREE}
\chapter{Preliminary Design/Decomposition}
...
\wepsfiga{figure-3-1}{Pools of thought not yet linked}
...
\wepsfig{figure-3-2}{Generalized decomposition of the Tiny Editor problem.}
...
\emph{To Overwrite:}&\\
Store new character into&                  F U N \fwbox{\pointto{K}} T I O N A L I T Y\\
byte pointer to by pointer&                F U N \fwbox{\pointto{C}} T I O N A L I T Y\\
Advance pointer (unless at end of field)&  F U N C \pointto{T} I O N A L I T Y\\[2ex]

\emph{To Delete:}&\\
Copy leftwards, by one place, the string&         F U N C T I O N \pointto{S} \fwbox{A L I T Y}\\
beginning one place to the right of the pointer.& F U N C T I O N \fwbox{\pointto{A} L I T Y} Y\\
Store a ``blank'' into the last&                  F U N C T I O N \pointto{A} L I T Y \fwbox{\newlength{\charheight}\settoheight{\charheight}{K}\rule{0cm}{\charheight} }\\
position on the line.&\\[2ex]

% Note, pointers-and-boxes omitted in this diagram for some reason.
\emph{To Insert:}&\\
Copy rightwards, by one place,&       F U N T I O N A L I T Y\\
the string beginning at the pointer&  F U N T T I O N A L I T Y\\
%!!'84 ed: the string beginning one place\\ to the right of the pointer.
Store new character into&             F U N C T I O N A L I T Y\\
byte pointed to by pointer.&          F U N T T I O N A L I T Y\\
...
\wepsfig{figure-3-3}{The traditional approach: view from the top.}
...
widest-angle view possible and examine the problem.  \Fig{figure-3-3}
...
Applying the notion of successive refinement, \fig{figure-3-4} shows the
...
\wepsfig{figure-3-4}{A structure for ``Process a Keystroke.''}
...
Following this notion, we draw yet another chart (\fig{figure-3-5}).
...
\wepsfig{figure-3-5}{Another structure for ``Process a Keystroke.''}
...
``Process a Character'' in \fig{figure-3-4}. Let's consider just one of the
seven possible execution paths, the one that happens if a printable
character is pressed.  In \fig{figure-3-6}(a) we see the original structural
path for a printable character.

Once we figure out the algorithms for overwriting and inserting
characters, we might refine it as shown in \fig{figure-3-6}(b). But look at
...
is unnecessary, and change the structure as shown in \fig{figure-3-6}(c).
...
\wepsfig{figure-3-6}{The same section, ``refined'' and ``optimized.''}
...
\fig{figure-3-7} (a) shows what happens to our first-pass design when we
...
\wepsfig{figure-3-7}{Adding refresh.}
...
each module must implement its own end of the interface (\fig{figure-3-8}).
...
\wepsfig{figure-3-8}{Traditional view of the interface as a junction.}
...
\wepsfig{figure-3-9}{Use of the interface component.}
...
elementary application (\fig{figure-3-10}a).
...
\wepsfig{figure-3-10}{Two ways to add advanced capabilities.}

implemented as an enhancement to some private, root function in the
elementary part of the application (\fig{figure-3-10}b). By being able to
...
\begin{verbatim}
HEX
COOO CONSTANT FIRST-ARRAY ( 8 bytes)
COO8 CONSTANT SECOND-ARRAY ( 6 bytes)
COOC CONSTANT THIRD ARRAY ( 100 bytes)
\end{verbatim}
...
\begin{verbatim}
>RAM 9 CONSTANT FIRST-ARRAY 8 >RAM +!
>RAM 8 CONSTANT SECOND-ARRAY 6 >RAM +!
...
\begin{verbatim}
FOOO >RAM ! ( EFFF, last byte, plus one)
: THERE ( -- address of next free byte in RAM)
  >RAM & ;
